    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts):
            ...
            if len(top_x) > 0:
                ...
                current_state = expert_layer(
                    hidden_states[top_x].reshape(-1, hidden_dim)
                ).to(hidden_states.device)
                current_hidden_states = (
                    current_state
                    * routing_weights[top_x, idx.to(hidden_states.device), None]
                )
        ...
        if use_fused_moe:
            ...
            if getattr(self.config, "use_shared_expert", False):
                expert_mapping.append(
                    ("shared_expert.gate_up_proj.0", "shared_expert_gate_proj", 0, 0)
                )
                expert_mapping.append(
                    ("shared_expert.gate_up_proj.1", "shared_expert_up_proj", 0, 1)
                )
                expert_mapping.append(
                    ("shared_expert.down_proj", "shared_expert_down_proj", 0, 0)
                )
                expert_mapping.append(
                    ("shared_expert_gate", "shared_expert_gate", 0, 0)
                )
