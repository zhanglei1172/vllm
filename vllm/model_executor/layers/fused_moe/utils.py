...
def _int8_quantize(
    ...
    if block_shape is None:
        if per_act_token_quant:
            ...
            assert A_scale is not None, "A_scale must be provided for static per-tensor quantization"
            assert A_scale.numel() == 1, "A_scale must be a single value for static per-tensor quantization"
            A = (A / A_scale).round().clamp(-128, 127).to(torch.int8)
    else:
